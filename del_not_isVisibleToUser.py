    ...
    if isinstance(json_data, dict):
        if "children" in json_data:
            ...
            for child in children:
                if "children" in child and not child["isVisibleToUser"]:
                ...
                elif "children" in child and child["isVisibleToUser"]:
                    tmp = merge_nested_json(child)
                    if isinstance(tmp, dict):
                        merged_list.append(tmp)
                    else:
                        for item in tmp:
                            merged_list.append(item)
                elif "children" not in child and child["isVisibleToUser"]:
                    merged_list.append(child)
            if json_data["isVisibleToUser"]:
...
def delete_not_isVisibleToUser_views(root_folder):
...
folder = './TestCap'
delete_not_isVisibleToUser_views(folder)
